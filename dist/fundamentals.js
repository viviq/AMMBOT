 ...
 */
export function startFundamentalsPolling(cfg, tokens, onUpdate) {
    ...
    if (api.provider === 'coingecko') {
        ...
        const poll = async () => {
            try {
                const url = `https://api.coingecko.com/api/v3/simple/price?ids=${encodeURIComponent(ids.join(','))}&vs_currencies=usd&include_market_cap=true&include_24hr_change=true`;
                const res = await fetch(url, { cache: 'no-store' });
                if (!res.ok)
                    throw new Error(`HTTP ${res.status}`);
                const data = (await res.json());
                for (const [token, id] of Object.entries(idMap)) {
                    if (!tokensEff.includes(token))
                        continue;
                    const entry = data[id];
                    if (!entry)
                        continue;
