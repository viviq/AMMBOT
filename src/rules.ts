import { AppConfig, DecisionResult, WindowFeatures } from './types.js';

/**
 * 基于配置阈值的规则触发（作为候选集筛选）
 */
export function evaluateRules(cfg: AppConfig, f: WindowFeatures): DecisionResult {
  const triggers: DecisionResult['triggers'] = [];
  // 价格异常：绝对值超出阈值（例如 -5% 到 5% 之外）
  if (
    f.priceChangePct10m !== undefined &&
