...
async function main() {
    ...
    // 运行时基础面数据缓存（优先使用实时采集）
    const fundamentalsLatest = new Map();
    // 启动基础面轮询
    const whitelistTokens = (cfg.symbols.whitelist || []).map(tokenFromSymbol);
    let fundamentalsCtl = { stop: () => void 0 };
    ...
    connectMarkPriceArrayStream((tick) => {
        if (cfg.symbols.blacklist.includes(tick.symbol))
            return;
        ...
        if (dyn) {
            if (dyn.marketCapUSD !== undefined)
                f.marketCapUSD = dyn.marketCapUSD;
            if (dyn.priceChange24hPct !== undefined)
                f.priceChange24hPct = dyn.priceChange24hPct;
        }
        else {
            const fund = cfg.fundamentals?.[token];
            if (fund) {
                if (fund.marketCapUSD !== undefined)
                    f.marketCapUSD = fund.marketCapUSD;
                if (fund.priceChange24hPct !== undefined)
                    f.priceChange24hPct = fund.priceChange24hPct;
            }
        }
